...
public class JumpLampAnimator implements Animator {
	...
	private AnimationStep animateBase() {
		...
		double height = Math.exp(-heightSinus * 3.5);
		...
		Modification translate = new TranslateModification(0, height * 1.5 * jumpHeight, 0);
		steps.add(translate);
		...
		if (needToRotateBase > 0) {
			...
			if (needToRotateBase - 0.5 > 0) {
				Modification translateFeet = new TranslateModification(Math.sin(3 * (sinusDegree - 0.5)), 0, 0);
	...
	private AnimationStep animateLowerJoint() {
		...
		if (Math.sin(sinusDegree) >= 0) {
			rotateMod = new RotateModification(- rotate * 50, 0, 0, 1.0);
			currentRotationLowerJoint = rotate * 20;
		} else {
			rotateMod = new RotateModification(rotate * 50, 0, 0, 1.0);
			//Modification baseRotationCompensation = new TranslateModification(0, Math.sin(sinusDegree) * 2, 0);
	...
	private AnimationStep animateHead() {
		...
		return new AnimationStep(new Modification[]{});
	}
